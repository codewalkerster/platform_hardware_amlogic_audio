...
int Balance_parse_level(BalanceContext *pContext, int num, const char *buffer)
{
    ...
    for (i = 0; i < num; i++) {
        ...
        //ALOGD("%s: Level[%d] = %s -> %f", __FUNCTION__, i, Rch, data->usr_cfg.level[i]);
    }
    ...
    case BALANCE_PARAM_INDEX:
        ...
        for (int i = 0; i < data->usr_cfg.num ; i++) {
            *pTmp++ = data->usr_cfg.level[i];
            //ALOGE(" Level[%f]",data->usr_cfg.level[i]);
         }
        break;
    default:
...
int dump(BalanceContext *pContext __unused)
{
    return 0;
}
...
int BalanceLib_Create_3_1(const effect_uuid_t *uuid, int32_t sessionId, int32_t ioId, int32_t deviceId __unused, effect_handle_t *pHandle)
{
    return BalanceLib_Create(uuid, sessionId, ioId, pHandle);
}
...
audio_effect_library_t AUDIO_EFFECT_LIBRARY_INFO_SYM = {
    ...
    .create_effect_3_1 = BalanceLib_Create_3_1,
