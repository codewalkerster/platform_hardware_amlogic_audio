...
typedef struct HPEQContext_s {
    ...
    // when receive setting change from app,
...
const int32_t default_usr_cfg_5_band[] = {
     0,  0,  0,  0,  0,   /* standard */
     3,  5, -3,  5,  0,   /* music */
     0,  3,  3,  0,  0,   /* news */
     2,  6,  6,  2,  3,   /* movie */
...
const int32_t default_usr_cfg_7_band[] = {
     0,  0,  0,  0,  0,  0,  0,   /* standard */
     3,  4,  5,  0, -3,  0,  5,   /* music */
     0,  0,  3,  3,  3,  0,  0,   /* news */
     2,  4,  6,  6,  4,  3,  2,   /* movie */
     6,  2, -2, -2,  0,  3,  3,   /* game */
...
const int32_t default_usr_cfg_9_band[] = {
     0,  0,  0,  0,  0,  0,  0,  0,  0,   /* standard */
     3,  4,  5,  0, -3, -4,  0,  3,  3,   /* music */
     0,  0,  3,  3,  3,  0,  0,  0,  0,   /* news */
     2,  4,  6,  6,  6,  4,  2,  3,  3,   /* movie */
     6,  2,  0,  0, -2,  0,  2,  2,  3,   /* game */
...
int HPEQ_init(HPEQContext *pContext)
{
    ...
    for (int i = 0; i < 9; i++) {
        data->cfg.band[i] = 0/*data->usr_cfg[(count >> LSR) + 1].band[i]*/;
    }
...
int HPEQ_reset(HPEQContext *pContext __unused, int bandSum)
{
    HPEQ_reset_api(bandSum);
    return 0;
}
...
int HPEQ_getParameter(HPEQContext *pContext, void *pParam, size_t *pValueSize, void *pValue)
{
    ...
    case HPEQ_PARAM_BAND_NUMBER:
        ...
        value = data->band_num;
        *(int32_t *) pValue = value;
        ALOGD("%s: Get band number -> %d", __FUNCTION__, value);
        if (value == HPEQ_5_BAND) {
            ALOGD("%s: HPEQ_v1.2-2022/11/10.Five bands are 120HZ, 500HZ, 1.5KKHZ, 5KHz, 10KHZ", __FUNCTION__);
        } else if (value == HPEQ_7_BAND) {
            ALOGD("%s: HPEQ_v1.2-2022/11/10.Seven bands are 100HZ, 250HZ, 600HZ, 1KHZ, 2.5KKHZ, 6KHz, 10KHZ", __FUNCTION__);
        } else if (value == HPEQ_9_BAND) {
            ALOGD("%s: HPEQ_v1.2-2022/11/10.Nine bands are 100HZ, 250HZ, 500HZ, 800HZ, 1KHZ, 2.5KKHZ, 5KHz, 8KHZ, 10KHZ", __FUNCTION__);
        }
        break;
    ...
    case HPEQ_PARAM_EFFECT_CUSTOM:
        if (*pValueSize < sizeof(HPEQcfg_8bit_s) || data->usr_cfg == NULL) {
            ALOGE("%s: value_size = %zu, size of HPEQcfg_8bit_s = %zu, usr_cfg is %s", __FUNCTION__,
                *pValueSize, sizeof(HPEQcfg_8bit_s), (data->usr_cfg == NULL) ? "NULL" : "OK");
            *pValueSize = 0;
            return -EINVAL;
        }

        custom_value.band[0] = (signed char)data->usr_cfg[(data->mode_num - 1) * data->band_num];
        custom_value.band[1] = (signed char)data->usr_cfg[(data->mode_num - 1) * data->band_num + 1];
        custom_value.band[2] = (signed char)data->usr_cfg[(data->mode_num - 1) * data->band_num + 2];
        custom_value.band[3] = (signed char)data->usr_cfg[(data->mode_num - 1) * data->band_num + 3];
        custom_value.band[4] = (signed char)data->usr_cfg[(data->mode_num - 1) * data->band_num + 4];
        if (data->band_num == HPEQ_7_BAND || data->band_num == HPEQ_9_BAND) {
            custom_value.band[5] = (signed char)data->usr_cfg[(data->mode_num - 1) * data->band_num + 5];
            custom_value.band[6] = (signed char)data->usr_cfg[(data->mode_num - 1) * data->band_num + 6];
        }
        if (data->band_num == HPEQ_9_BAND) {
            custom_value.band[7] = (signed char)data->usr_cfg[(data->mode_num - 1) * data->band_num + 7];
            custom_value.band[8] = (signed char)data->usr_cfg[(data->mode_num - 1) * data->band_num + 8];
        }
...
int HPEQ_setParameter(HPEQContext *pContext, void *pParam, void *pValue)
{
    ...
    case HPEQ_PARAM_BAND_NUMBER:
        value = *(int32_t *)pValue;
        if ((HPEQ_5_BAND != value) && (HPEQ_7_BAND != value) && (HPEQ_9_BAND != value)) {
            ALOGE("%s: incorrect band number %d", __FUNCTION__, value);
            return -EINVAL;
        }
        if (data->band_num < value) {
            data->usr_cfg = (int *)realloc((void *)data->usr_cfg, data->mode_num * value * (sizeof(int)));
            if (!data->usr_cfg) {
                ALOGE("%s: realloc failed", __FUNCTION__);
                return -EINVAL;
            } else {
                ALOGD("%s: realloc success", __FUNCTION__);
            }
        }
        data->band_num = value;
        if (data->band_num == HPEQ_5_BAND) {
            memcpy((void *) data->usr_cfg, (void *) &default_usr_cfg_5_band,
                data->band_num * data->mode_num * sizeof(int));
        } else if (data->band_num == HPEQ_7_BAND) {
            memcpy((void *) data->usr_cfg, (void *) &default_usr_cfg_7_band,
                data->band_num * data->mode_num * sizeof(int));
        } else if (data->band_num == HPEQ_9_BAND) {
            memcpy((void *) data->usr_cfg, (void *) &default_usr_cfg_9_band,
                data->band_num * data->mode_num * sizeof(int));
        }

        HPEQ_init_api((void *)data, value);
        ALOGD("%s: Set band num  -> %d", __FUNCTION__, value);
        break;
    case HPEQ_PARAM_EFFECT_MODE:
        value = *(int32_t *)pValue;
        ...
        if (pContext->bUseFade) {
            ...
            for (i = 0; i < data->band_num; i++) {
                ...
                HPEQ_setBand_api(data->usr_cfg[data->mode * data->band_num + i], i + 1, data->band_num);
            }
        }
        break;
    case HPEQ_PARAM_EFFECT_CUSTOM:
        custom_value = *(HPEQcfg_8bit_s *)pValue;
        data->usr_cfg[(data->mode_num - 1) * data->band_num] = (signed int)custom_value.band[0];
        data->usr_cfg[(data->mode_num - 1) * data->band_num + 1] = (signed int)custom_value.band[1];
        data->usr_cfg[(data->mode_num - 1) * data->band_num + 2] = (signed int)custom_value.band[2];
        data->usr_cfg[(data->mode_num - 1) * data->band_num + 3] = (signed int)custom_value.band[3];
        data->usr_cfg[(data->mode_num - 1) * data->band_num + 4] = (signed int)custom_value.band[4];
        if (data->band_num == HPEQ_7_BAND || data->band_num == HPEQ_9_BAND) {
            data->usr_cfg[(data->mode_num - 1) * data->band_num + 5] = (signed int)custom_value.band[5];
            data->usr_cfg[(data->mode_num - 1) * data->band_num + 6] = (signed int)custom_value.band[6];
        }
        if (data->band_num == HPEQ_9_BAND) {
            data->usr_cfg[(data->mode_num - 1) * data->band_num + 7] = (signed int)custom_value.band[7];
            data->usr_cfg[(data->mode_num - 1) * data->band_num + 8] = (signed int)custom_value.band[8];
        }
        ...
        ALOGD("%s: Custom Set Mode -> %d, bUseFade = %d", __FUNCTION__, pContext->modeValue , pContext->bUseFade);
        if (pContext->bUseFade && needFade) {
            ...
            for (i = 0; i < data->band_num; i++) {
                ...
                HPEQ_setBand_api(data->usr_cfg[(data->mode_num - 1) * data->band_num + i], i + 1, data->band_num);
            }
        }
    break;
    default:
        ...
        void *pCmdData, uint32_t *replySize, void *pReplyData)
{
    ...
    case EFFECT_CMD_RESET:
        HPEQ_reset(pContext, data->band_num);
        break;
    ...
    case EFFECT_CMD_SET_PARAM:
        if (pCmdData == NULL ||
            ...
            ALOGE("%s: cmdsize %d effect_param_t size %d", __FUNCTION__, cmdSize, (int)sizeof(effect_param_t));
            return -EINVAL;
        }
        p = (effect_param_t *)pCmdData;
...
int HPEQLib_Create(const effect_uuid_t *uuid, int32_t sessionId __unused, int32_t ioId __unused, effect_handle_t *pHandle)
{
    ...
    if (HPEQ_load_ini_file(pContext) < 0) {
        ...
        pContext->gHPEQdata.mode_num = 6;
        pContext->gHPEQdata.band_num = HPEQ_5_BAND;

        if (pContext->gHPEQdata.usr_cfg == NULL) {
        ...
        if (pContext->gHPEQdata.band_num == HPEQ_5_BAND) {
            memcpy((void *) pContext->gHPEQdata.usr_cfg, (void *) &default_usr_cfg_5_band,
                pContext->gHPEQdata.band_num * pContext->gHPEQdata.mode_num * sizeof(int));
        }

        /*for (int i = 0; i < pContext->gHPEQdata.band_num * 6; i++) {
            ALOGD("%s: default i: %d , usr cfg: %d  ", __FUNCTION__, i, pContext->gHPEQdata.usr_cfg[i]);
        }*/
...
int HPEQLib_Create_3_1(const effect_uuid_t *uuid, int32_t sessionId, int32_t ioId, int32_t deviceId __unused, effect_handle_t *pHandle)
{
    return HPEQLib_Create(uuid, sessionId, ioId, pHandle);
}
...
audio_effect_library_t AUDIO_EFFECT_LIBRARY_INFO_SYM = {
    ...
    .create_effect_3_1 = HPEQLib_Create_3_1,
