...
#define LOG_TAG "audio_hw_hal_spdifout"
//#define LOG_NDEBUG 0
...
void aml_audio_select_src_to_hdmi(enum AML_SRC_TO_HDMITX src_select)
{
    struct aml_audio_device *aml_dev = (struct aml_audio_device *)adev_get_handle();
...
int aml_audio_spdifout_open(void **pphandle, spdif_config_t *spdif_config)
{
    ...
    if (audio_is_linear_pcm(spdif_config->audio_format) && (EARC_DEVICE != device_id)) {
        if (audio_channel_count_from_out_mask(phandle->channel_mask) != phandle->out_data_ch) {
            int last_device_id = device_id;
            audio_channel_mask_t out_channel_mask = audio_channel_out_mask_from_count(phandle->out_data_ch);

            AM_LOGI("data_ch %d, change channel_mask 0x%x to 0x%x", phandle->out_data_ch, \
                phandle->channel_mask, out_channel_mask);
            phandle->channel_mask = out_channel_mask;
            spdif_config->channel_mask = out_channel_mask;

            device_id = select_digital_device(phandle);
            if (device_id != last_device_id) {
                AM_LOGI("device_id change from %d to %d", last_device_id, device_id);
            }
        }
    }
    ...
    if (!alsa_handle) {
        ...
        if (phandle->spdif_port == PORT_SPDIF) {
            ...
            ALOGI("%s set i2s:%d to hdmi format 0x%x", __func__,
                hdmitx_src - AML_TDM_A_TO_HDMITX, aml_spdif_format);
...
int aml_audio_spdifout_process(void *phandle, void *buffer, size_t byte)
{
    ...
    if (eDolbyDcvLib == aml_dev->dolby_lib_type || AML_DIGITAL_AUDIO_MODE_BYPASS == aml_dev->digital_audio_mode) {
        ...
        if ((spdifout_phandle->spdif_port == PORT_EARC || spdifout_phandle->spdif_port == PORT_I2S2HDMI)
            ...
            if (spdifout_phandle->need_extend_channel && spdifout_phandle->out_data_ch != 0 && spdifout_phandle->in_data_ch != 0) {
                ...
                output_buffer_bytes = adjust_channels((const void*) output_buffer,
                                                      ...
                                                      spdifout_phandle->out_data_ch,
                                                      sizeof(int16_t),
                                                      output_buffer_bytes);
                if (output_buffer_bytes == 0) {
                    ALOGE("%s adjust_channels errors", __func__);
                    return -1;
                }
...
int aml_audio_spdifout_close(void *phandle)
{
    ...
    if (spdifout_phandle->temp_buf != NULL) {
        aml_audio_free(spdifout_phandle->temp_buf);
        spdifout_phandle->temp_buf = NULL;
    }
