...
int aml_hwsync_wrap_get_pts(audio_hwsync_t *p_hwsync, uint64_t *pts)
{
    ...
    ret = mediasync_wrap_getMediaTime(p_hwsync->mediasync, systemTime(SYSTEM_TIME_MONOTONIC) / 1000LL, &timeus, 0);
    if (!ret) {
        return ret;
    }
...
bool aml_audio_hwsync_update_threshold(audio_hwsync_t *p_hwsync)
{
     ...
     */
    if (adev && (AML_DIGITAL_AUDIO_MODE_BYPASS == adev->digital_audio_mode || eDolbyMS12Lib != adev->dolby_lib_type)) {
...
void aml_hwsync_wrap_is_amaster(audio_hwsync_t *p_hwsync, bool *b_amaster) {
    ...
    sync_mode mode = MEDIA_SYNC_MODE_MAX;
    ret = mediasync_wrap_getSyncMode(p_hwsync->mediasync, &mode);
    ALOGV("%s ret =%d sync mode =%d", __func__, ret, mode);
