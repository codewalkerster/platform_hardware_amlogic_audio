    ...
    */
    ...
    while (!feof(fp)) {
        ...
        if (found_index >= 0) {
            ...
            AM_LOGE("can not store this vm item %s\n", vm_item.filepath);
            last_so_filepath = NULL;
            continue;
        }
...
static aml_vm_lock_item *aml_alloc_vm_lock_item(aml_vm_item *p_item)
{
    ...
    if (p_lock_item == NULL) {
        AM_LOGE("calloc %zu bytes fail !", sizeof(aml_vm_lock_item));
        return NULL;
    }
    ...
    if (filepath == NULL) {
        AM_LOGE("calloc %d bytes fail !", path_len);
    ...
    if (p_dst_range == NULL) {
        ...
        p_lock_item = NULL;
        filepath = NULL;
        AM_LOGE("calloc %zu bytes fail !", range_num * sizeof(aml_vm_range));
        return NULL;
    }
...
void aml_load_lock_lib_address(void)
{
    ...
    bool ret = false;
    const int MAX_VM_NUM = 240;
    aml_vm_item *p_item = NULL;
    aml_vm_item *map_vm_table = NULL;

    if (enable_mlock_specific_lib() == false) {
        AM_LOGI("mlock_specific_lib is disabled");
        return;
    }

    if (aml_mlock_table != NULL) {
        AM_LOGI("aml_mlock_table %p, already load !", aml_mlock_table);
        return;
    }

    map_vm_table = aml_audio_calloc(MAX_VM_NUM, sizeof(aml_vm_item));
    if (map_vm_table == NULL) {
        AM_LOGE("calloc %zu bytes fail !", MAX_VM_NUM * sizeof(aml_vm_item));
        return;
    }
    ret = aml_parse_proc_maps("/proc/self/maps", map_vm_table, MAX_VM_NUM);
    if (ret != true) {
        AM_LOGE("aml_parse_proc_maps /proc/self/maps fail !");
        return;
    }

    int mlock_so_num = sizeof(hal_mlock_so)/sizeof(hal_mlock_so[0]);
    AM_LOGI("mlock_so_num %d", mlock_so_num-1);
    aml_mlock_table = aml_audio_calloc(mlock_so_num, sizeof(aml_vm_lock_item *));
    if (aml_mlock_table == NULL) {
        AM_LOGE("calloc %zu bytes fail !", mlock_so_num * sizeof(aml_vm_lock_item *));
    ...
    while (index < MAX_VM_NUM) {
        ...
        if (check_mlock_path(p_item->filepath, hal_mlock_so)) {
