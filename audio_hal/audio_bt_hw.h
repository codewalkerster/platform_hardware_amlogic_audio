...
class BluetoothAudioPort {
  ...
  ***/
   ...
   ***/
  virtual void SetState(BluetoothStreamState state __unused) {}

  virtual bool IsA2dp() const { return false; }

  virtual bool IsLeAudio() const { return false; }

  virtual bool GetPreferredDataIntervalUs(size_t* interval_us __unused) const {
    return false;
  };

  virtual size_t WriteData(const void* buffer __unused, size_t bytes __unused) const {
    return 0;
  };
  virtual size_t ReadData(void* buffer __unused, size_t bytes __unused) const { return 0; };
};
