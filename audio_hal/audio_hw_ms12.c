...
#define LOG_TAG "audio_hw_hal_ms12v1"
//#define LOG_NDEBUG 0
#define __USE_GNU
...
#define AUDIO_HAL_VERSION_1 (1)//callback own 4 parameters
/*########################### IMPORTANT AUDIO HAL VERSION #####################################*/
...
int dolby_ms12_register_callback(struct aml_stream_out *aml_out)
{
    ...
    if (ms12->output_config & MS12_OUTPUT_MASK_SPEAKER) {
        ret = dolby_ms12_register_dap_pcm_callback(ms12_output, (void *)aml_out);
        ALOGI("%s() dolby_ms12_register_pcm_callback return %d", __FUNCTION__, ret);
    }
    if (ms12->output_config & MS12_OUTPUT_MASK_STEREO) {
        ret = dolby_ms12_register_pcm_callback(ms12_output, (void *)aml_out);
        ALOGI("%s() dolby_ms12_register_pcm_callback return %d", __FUNCTION__, ret);
    }
    if (ms12->output_config & MS12_OUTPUT_MASK_DDP) {
        ret = dolby_ms12_register_bitstream_callback(ms12_output, (void *)aml_out);
        ALOGI("%s() dolby_ms12_register_bitstream_callback return %d", __FUNCTION__, ret);

    }
    if (ms12->output_config & MS12_OUTPUT_MASK_DD) {
        ret = dolby_ms12_register_spdif_bitstream_callback(ms12_output, (void *)aml_out);
...
void set_ms12_main_audio_mute(struct dolby_ms12_desc *ms12, bool b_mute, unsigned int duration)
{
    ...
    */
    if (b_mute) {
        sprintf(parm, "%s %d,%d,%d", "-sys_prim_mixgain", -96, duration, 0);
    } else {
    ...
    , int input_sample_rate)
{
    ...
    */
    ...
    if (continuous_mode(adev)) {
        ...
        *to determine use the spdif encoder or not.
        */
    ...
    if (ms12->dolby_ms12_enable) {
        /* set the audio hal version */
        dolby_ms12_set_audio_hal_version(AUDIO_HAL_VERSION_1);
    ...
    */
    if (adev->audio_patch_2_af_stream || (adev->cur_out_devices & AUDIO_DEVICE_OUT_HDMI_ARC)
...
bool is_ms12_passthrough(struct audio_stream_out *stream) {
    ...
    ALOGV("bypass_ms12 =%d hdmi format =%d optical format =0x%x 0x%x",
        bypass_ms12, adev->digital_audio_mode, ms12->optical_format, aml_out->hal_internal_format);
    return bypass_ms12;
}
    ...
    if (ms12->dolby_ms12_enable) {
        ...
        */
            ...
            if (output_format == AUDIO_FORMAT_E_AC3
                ...
                ALOGV("Input size =%d used_size =%d output size=%d rate=%d internal format=0x%x rate=%d",
        ...
        if (ms12->dual_decoder_support == true) {
            ...
            if (!is_ad_data_available(adev->digital_audio_mode)) {
                if (ms12_hal_format == AUDIO_FORMAT_AC3) {
    ...
    if (ms12->is_bypass_ms12
        ...
        if (bytes != 0 && buffer != NULL) {
            if (bitstream_out->spdifout_handle != NULL && bitstream_out->audio_format != output_format) {
                ALOGI("spdif output format changed from =0x%x to 0x%x", bitstream_out->audio_format, output_format);
                aml_audio_spdifout_close(bitstream_out->spdifout_handle);
    ...
    if (ms12 && buf && (n_bytes_buf > 0) && ms12_info) {
        ...
        if (is_beep_frame) {
            ...
            if (is_pts_available && is_pts_in_one_beep) {
               ...
               ALOGI("count %llu out_frame_pts %lld ms decoder out synced frame at %d ms pre_zero_samples %d actual_synced_frame %d ms master_pcm_frames %llu",
...
int dolby_ms12_get_latency(audio_format_t output_format, int pcm_type)
{
    ...
    if (output_format == AUDIO_FORMAT_PCM_16_BIT) {
        if (pcm_type == NORMAL_LPCM)
            ...
            dolby_ms12_get_latency_for_dap_speaker_out(&ms12_total_delay_frames);
    }
    else if (output_format == AUDIO_FORMAT_AC3) {
        dolby_ms12_get_latency_for_dd_out(&ms12_total_delay_frames);
    }
    else if (output_format == AUDIO_FORMAT_E_AC3) {
        dolby_ms12_get_latency_for_ddp_out(&ms12_total_delay_frames);
    }
    else {
        dolby_ms12_get_latency_for_stereo_out(&ms12_total_delay_frames);
    }
...
void ms12_output_update_audio_pts(struct audio_stream_out *stream, aml_ms12_dec_info_t *ms12_info, void *buffer, size_t size)
{
    ...
    if (patch && patch->dtvsync && (adev->sink_format == output_format) && !ms12->is_bypass_ms12) {
        ...
        if (patch->cur_package && adev->debug_flag && (patch->cur_package->pts != ULLONG_MAX)) {
            ...
            ALOGI("%s package pts %llx ms12_main_apts %llx pcm-duration %x cur_outapts %llx, alsa_latency %x ms12_tuning_delay_pts %x start-pts %llx end-pts %llx\n",
    ...
    */
    ...
    if (audio_is_linear_pcm(output_format)) {
        if (ms12_info->pcm_type == DAP_LPCM) {
            ...
            dap_pcm_output_l(buffer, priv_data, size);
        } else {
            if (get_debug_value(AML_DEBUG_AUDIOHAL_LEVEL_DETECT)) {
                check_audio_level("ms12_stereo_pcm", buffer, size);
            }
            stereo_pcm_output_l(buffer, priv_data, size);
        }
    } else {
        if (output_format == AUDIO_FORMAT_E_AC3) {
            bitstream_output_l(buffer, priv_data, size);
        } else if (output_format == AUDIO_FORMAT_AC3) {
            spdif_bitstream_output_l(buffer, priv_data, size);
        }
        else {
...
{
    struct dolby_ms12_desc *ms12 = &(adev->ms12);
    bool is_dap_enable = (adev->cur_out_devices & AUDIO_DEVICE_OUT_SPEAKER) && (!adev->ms12.dap_bypass_enable);
...
unsigned long long dolby_ms12_get_main_bytes_consumed(struct audio_stream_out *stream) {
    ...
    if (adev->debug_flag > 1) {
        ALOGI("%s main bytes offset =%lld consumed =%lld total =%lld",
...
bool is_ms12_output_compatible(struct audio_stream_out *stream, audio_format_t new_sink_format, audio_format_t new_optical_format) {
    ...
    if (adev->digital_audio_mode == AML_DIGITAL_AUDIO_MODE_BYPASS || adev->digital_audio_mode == AML_DIGITAL_AUDIO_MODE_PCM) {
