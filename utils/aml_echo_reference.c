...
static int echo_reference_get_next_buffer(struct resampler_buffer_provider *buffer_provider,
...
static void echo_reference_release_buffer(struct resampler_buffer_provider *buffer_provider,
                                  struct resampler_buffer* buffer)
{
    struct echo_reference *er;
...
//    ALOGV("echo_reference_read() %d frames", buffer->frame_count);
    ...
    if (er->frames_in < buffer->frame_count) {
        uint32_t timeoutMs = (uint32_t)((1000 * er->wr_curr_frame_size) / er->wr_sampling_rate * 2);
        struct timespec ts = {0, 0};
        ALOGD("shuai, wait time %d", timeoutMs);
