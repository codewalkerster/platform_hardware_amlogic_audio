...
#define LOG_TAG "audio_hw_utils_parserheaac"
//#define LOG_NDEBUG 0
...
int aml_heaac_parser_open(void **pparser_handle)
{
    ...
    parser_handle->parsed_frame_flag = 0;
...
int aml_heaac_parser_reset(void *parser_handle)
{
    ...
    if (heaac_parser_handle) {
        ...
        heaac_parser_handle->parsed_frame_flag = 0;
    }
    ALOGI("%s  exit", __func__);
    return 0;
}
...
int aml_heaac_parser_sniff_format(void *in_buffer, int32_t numBytes)
{
     ...
     while (!loas_heaac_info.is_loas) {
        ...
        if (ret != -1 && ret < numBytes - buffer_offset) {
            buffer_offset += ret;
...
resync:
    ...
    if (heaac_parser_handle->status == PARSER_SYNCING) {
        ...
        if (need_size > 0) {
            ...
            /*make sure the remain buf has HEAAC_HEADER_SIZE bytes*/
            memcpy(parser_buf + heaac_parser_handle->buf_remain, buffer + buf_offset , need_size);
            heaac_parser_handle->buf_remain = CLIPINT((int64_t)heaac_parser_handle->buf_remain + (int64_t)need_size);
            buf_offset += need_size;
            buf_left = numBytes - buf_offset;
        }
    }
...
error:
    ...
    *out_size   = 0;
    *used_size = numBytes;
    if (heaac_parser_handle->parsed_frame_flag == 0) {
        heaac_parser_handle->format_checked_flag = 0;
    }
