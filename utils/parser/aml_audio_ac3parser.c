...
int aml_ac3_parser_open(void **pparser_handle, void *pParserConfig)
{
...
int ac3_parsing_data_process(void *phandle, const void *inABuffer, void *outABuffer, void *parser_callback)
{
    ...
    struct aml_ac3_parser *pParserHanle = (struct aml_ac3_parser *)phandle;
    ...
    do {
        ...
        AM_LOGV(" is_sub_parser:%d  phandle:%p inBuf:%p inSize(leftBytes):%d %d,  used_bytes:%d totalUsedBytes:%d outBuffer:%p out_frame_size:%d  ac3_info.frame_size:%d",
            is_sub_parser, phandle, inBuf, inSize, leftBytes, usedBytes, totalUsedBytes, outBuffer, outBytes, ac3_info.frame_size);
