...
{
    return aml_mixer_ctrl_get_int(mixer_handle, AML_MIXER_ID_EARCRX_AUDIO_CODING_TYPE);
}
...
int earc_coding_type_to_codec(enum earc_audio_type type)
{
    ...
    switch (type) {
        case EARC_AC3:
        case EARC_AC3_LAYOUT_B:
            audio_codec = AC3;
            break;
        case EARC_EAC3:
        case EARC_EAC3_LAYOUT_B:
            audio_codec = EAC3;
            break;
        case EARC_MLP:
        case EARC_MLP_LAYOUT_B:
            audio_codec = MAT;
            break;
        case EARC_DTS:
        case EARC_DTS_LAYOUT_B:
            audio_codec = DTS;
            break;
        case EARC_DTS_HD:
        case EARC_DTS_HD_LAYOUT_B:
            audio_codec = DTSHD;
            break;
        case EARC_PAUSE:
            //audio_code = PAUSE;
...
int pcm_coding_type_to_channels(enum earc_audio_type type)
{
    ...
    case EARC_MULTICH_8CH_LPCM:
        channels = 8;
        break;
    case EARC_MULTICH_16CH_LPCM:
        channels = 16;
        break;
    case EARC_MULTICH_32CH_LPCM:
        channels = 32;
        break;
    case EARC_STEREO_LPCM:
    case EARC_MULTICH_2CH_LPCM:
    default:
        channels = 2;
        break;
    }
...
int audio_raw_data_parse(audio_type_parse_t *status, void *buffer, size_t bytes)
{
    ...
    audio_type_status->cur_audio_type = audio_type_parse(buffer, bytes, &(audio_type_status->package_size),
